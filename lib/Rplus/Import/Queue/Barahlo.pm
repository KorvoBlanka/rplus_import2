package Rplus::Import::Queue::Barahlo;
...
sub _get_url_list {
    ...
    for(my $i = 1; $i <= $page_count; $i ++) {
        ...
        $dom->find('div[class~="wrap-items realty"]')->each (sub {
            my $item_id = $_->{id};
            my $item_url = $_->at('a[class~="ads-title-link"]')->{href};
            my $date_str = $_->at('td[class~="ob_descr"]')->at('p[class="grey"]')->text;
